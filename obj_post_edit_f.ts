...
export let functions = {
     ...
     surround_post_text_in_tags_controller(rows: HTMLElement[], array_of_search_key: string[]) {
          ...
          for (let i = 0; i < rows.length; i++) {
               ...
               let text_with_symbols_tags = escaped_itext_row.replace(regexp, function (search_key: string) {
                    let reg_verb = new RegExp(patterns.pattern_verb, 'gmu'),
                         ...
                         reg_words_in_tag = new RegExp(array_of_search_key.join('|'), 'gu'),
                    
                         is_exist_tags:boolean = reg_tag.test(search_key),
                         is_exist_tags_action:boolean = reg_verb.test(search_key),
                         is_exist_finding_word:boolean = reg_words_in_tag.test(search_key);

                        /* #edit combine or #remove
                    var reg_words_in_tag = new RegExp(array_of_search_key.join('|'), 'gu');
          ...
          //#edit not allow return undefined
          if(chain_fathers == ""){
               chain_fathers = "---";
          }
